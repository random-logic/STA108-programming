...
# 3.19
This difference can arise because $\hat{Y_i} = b_0 + b_1 x_i$ does not depend on $\epsilon_i$, while $Y_i = \beta_0 + \beta_1 x_i + \epsilon_i$ depends on $\epsilon_i$. The residual is dependent on $\epsilon_i$. The plot with residual against $Y_i$ is more meaningful and useful for making interpretations.
